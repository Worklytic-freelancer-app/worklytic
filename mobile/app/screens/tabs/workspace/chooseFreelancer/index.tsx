...
export default function ChooseFreelancer() {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([refetchProject(), refetchFeatures(), refetchUsers()]);
    setRefreshing(false);
  ...
  const updateProjectStatus = useMutation<Project, Record<string, unknown>>({
    endpoint: "projects",
    method: "PUT",
    requiresAuth: true,
  ...
  const confirmMarkAsCompleted = (featureId: string) => {
    ...
    const promises = [
      ...
      // Update status project menjadi open
      updateProjectStatus.mutateAsync({
        status: "completed",
        customEndpoint: `projects/${projectId}`,
      }),
    ];
      ...
      <View style={styles.applicantCard}>
        <View style={styles.applicantHeader}>
          <TouchableOpacity onPress={() => item.freelancer && navigation.navigate("FreelancerDetails", { freelancerId: item.freelancer._id })}>
            <ImageWithSkeleton source={{ uri: item.freelancer?.profileImage }} style={styles.applicantAvatar} />
          </TouchableOpacity>
          ...
          <View style={styles.menuContainer}>
            <TouchableOpacity style={styles.menuButton} onPress={() => setOpenMenuId(isMenuOpen ? null : item._id)}>
        ...
        {item.content && item.content.length > 0 && <Text style={styles.coverLetter}>{item.content[0].description || "Tidak ada deskripsi"}</Text>}

        {item.status === "pending" ? (
          <View style={styles.actionButtons}>
            <TouchableOpacity style={[styles.actionButton, styles.rejectButton]} onPress={() => handleRejectFreelancer(item._id)} disabled={actionType.id === item._id}>
              {isRejectLoading ? <ActivityIndicator size="small" color="#EF4444" /> : <Text style={styles.rejectButtonText}>Reject</Text>}
            </TouchableOpacity>

            <TouchableOpacity style={[styles.actionButton, styles.acceptButton]} onPress={() => handleAcceptFreelancer(item._id)} disabled={actionType.id === item._id}>
              {isAcceptLoading ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.acceptButtonText}>Accept</Text>}
            </TouchableOpacity>
          </View>
        ) : (
          <TouchableOpacity style={styles.viewButtonFull} onPress={() => navigation.navigate("WorkspaceDetails", { projectId, freelancerId: item.freelancer?._id })}>
      ...
      return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Maaf, proyek yang kamu cari tidak ditemukan. Proyek mungkin telah dihapus atau tidak dapat diakses.</Text>
            <TouchableOpacity style={[styles.postProjectButton, { backgroundColor: COLORS.error }]} onPress={() => navigation.goBack()}>
    ...
    if (applicants.length === 0) {
      return (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>Belum ada freelancer yang melamar untuk proyek ini. Tunggu beberapa saat atau coba perbarui detail proyek untuk menarik lebih banyak pelamar.</Text>
        </View>
      );
    }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <Confirmation visible={confirmation.visible} title={confirmation.title} message={confirmation.message} type={confirmation.type} onConfirm={confirmation.onConfirm || (() => setConfirmation((prev) => ({ ...prev, visible: false })))} />
    </View>
  );
}
