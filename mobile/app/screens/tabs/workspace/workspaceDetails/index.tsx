...
export default function WorkspaceDetails() {
  ...
  useEffect(() => {
    if (allFeatures.length > 0 && !featureId) {
      const feature = allFeatures.find((f) => f.projectId === projectId && f.freelancerId === freelancerId);
  ...
  const handleSendUpdate = async (content: string, attachments: Attachment[]) => {
    ...
    if (!featureId || !user?._id) {
      setConfirmation({
        visible: true,
        title: "Error",
        message: "Data is incomplete for sending an update",
        type: "error",
    ...
    try {
      ...
      setConfirmation({
        visible: true,
        title: "Error",
        message: err instanceof Error ? err.message : "Failed to send update",
        type: "error",
        onConfirm: () => setConfirmation((prev) => ({ ...prev, visible: false })),
      });
    } finally {
      ...
      <View key={url} style={styles.attachmentItem}>
        <View style={styles.attachmentContent}>
          ...
          <View style={styles.attachmentDetails}>
            ...
            <Text style={styles.attachmentMeta}>{type === "image" ? "Gambar" : "Dokumen"}</Text>
          </View>
        </View>
        <TouchableOpacity style={styles.downloadButton} onPress={() => handleDownload(url, filename)} disabled={isDownloading}>
          {isDownloading ? <ActivityIndicator size="small" color={COLORS.primary} /> : <Download size={20} color={COLORS.primary} />}
        </TouchableOpacity>
      </View>
    );
  };
  ...
  const renderHeader = () => {
    ...
    return (
      <View style={styles.projectInfoContainer}>
        <TouchableOpacity style={styles.projectInfo} onPress={navigateToProjectDetails}>
          <View style={styles.projectTitleContainer}>
            <Text style={styles.projectTitle} numberOfLines={2}>
              {projectFeature.project.title}
            </Text>
            <Text style={styles.projectBudget}>Rp{projectFeature.project.budget.toLocaleString("id-ID")}</Text>
          </View>
          <View style={styles.projectMeta}>
            <View style={styles.metaItem}>
              <Calendar size={16} color={COLORS.gray} />
              <Text style={styles.metaText}>{formatDate(projectFeature.project.createdAt)}</Text>
            </View>
            <View style={styles.statusBadge}>
              <Text style={styles.statusText}>{projectFeature.status}</Text>
            </View>
          </View>
        </TouchableOpacity>
      </View>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <FlatList
          ...
          <View style={styles.updateItem}>
            <View style={styles.updateHeader}>
              <View style={styles.userInfo}>
                <Image
                  source={{
                    uri: discussion.sender.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(discussion.sender.fullName)}&background=random`,
                  }}
                  style={styles.userAvatar}
                />
                <View>
                  <Text style={styles.userName}>{discussion.sender.fullName}</Text>
                  <View style={[styles.roleBadge, discussion.sender.role === "client" ? styles.clientBadge : styles.freelancerBadge]}>
                    <Text
                      style={[
                        styles.roleText,
                        {
                          color: discussion.sender.role === "client" ? COLORS.primary : "#0284c7",
                        },
                      ]}
                    >
            ...
            {discussion.description && <Text style={styles.updateContent}>{discussion.description}</Text>}
        ...
        contentContainerStyle={[styles.discussionsContent, { paddingBottom: paddingBottom }]}
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>There is no discussion for this project yet. Start a discussion by sending a message.</Text>
          </View>
        )}
        refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refreshData} colors={[COLORS.primary]} tintColor={COLORS.primary} progressBackgroundColor={COLORS.background} />}
      />

      {/* Input dengan KeyboardAvoidingView */}
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "position" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : 0} style={styles.keyboardAvoidingContainer}>
        <View style={[styles.inputContainer, { paddingBottom: insets.bottom > 0 ? insets.bottom : 16 }]}>
          <Input onSend={handleSendUpdate} loading={sendingUpdate} />
        </View>
