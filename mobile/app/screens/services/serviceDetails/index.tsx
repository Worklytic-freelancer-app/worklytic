...
interface ServiceWithFreelancer {
...
export default function ServiceDetails() {
  ...
  const [data, setData] = useState<ServiceWithFreelancer>({
    service: null,
    freelancer: null
  });
  ...
  const fetchServiceDetails = async () => {
    try {
      ...
      if (!serviceResult.success || !serviceResult.data) {
        setError(serviceResult.message || 'Failed to fetch service details');
        return;
      }
      ...
      const freelancerResponse = await fetch(`${baseUrl}/api/users/${service.freelancerId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      const freelancer = freelancerResult.success ? freelancerResult.data : null;
